...
Value createmultisig(const Array& params, bool fHelp)
{
    ...
    CPepeCoinAddress address(innerID);
...
Value getnewaddress(const Array& params, bool fHelp)
{
    ...
    return CPepeCoinAddress(keyID).ToString();
}


CPepeCoinAddress GetAccountAddress(string strAccount, bool bForceNew=false)
{
    ...
    return CPepeCoinAddress(account.vchPubKey.GetID());
}
...
Value getaddressesbyaccount(const Array& params, bool fHelp)
{
    ...
    Array ret;
    BOOST_FOREACH(const PAIRTYPE(CPepeCoinAddress, string)& item, pwalletMain->mapAddressBook)
    {
        const CPepeCoinAddress& address = item.first;
...
Value listaddressgroupings(const Array& params, bool fHelp)
{
    ...
    BOOST_FOREACH(set<CTxDestination> grouping, pwalletMain->GetAddressGroupings())
    {
        ...
        {
            Array addressInfo;
            addressInfo.push_back(CPepeCoinAddress(address).ToString());
            addressInfo.push_back(ValueFromAmount(balances[address]));
            {
                LOCK(pwalletMain->cs_wallet);
                if (pwalletMain->mapAddressBook.find(CPepeCoinAddress(address).Get()) != pwalletMain->mapAddressBook.end())
                    addressInfo.push_back(pwalletMain->mapAddressBook.find(CPepeCoinAddress(address).Get())->second);
            }
...
Value getreceivedbyaddress(const Array& params, bool fHelp)
{
    ...
    // Bitcoin address
    CPepeCoinAddress address = CPepeCoinAddress(params[0].get_str());
    CScript scriptPubKey;
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid PepeCoin address");
...
Value sendfrom(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 7)
        throw runtime_error(
            ...
            "3. amount                (numeric, required) The amount in PEPE. (transaction fee is added on top).\n"
    ...
    string strAccount = AccountFromValue(params[0]);
    CPepeCoinAddress address(params[1].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid PepeCoin address");
    CAmount nAmount = AmountFromValue(params[2]);
...
Value addmultisigaddress(const Array& params, bool fHelp)
{
    ...
    return CPepeCoinAddress(innerID).ToString();
}
...
Value ListReceived(const Array& params, bool fByAccounts)
{
    ...
    // Tally
    map<CPepeCoinAddress, tallyitem> mapTally;
    for (map<uint256, CWalletTx>::iterator it = pwalletMain->mapWallet.begin(); it != pwalletMain->mapWallet.end(); ++it)
    {
        const CWalletTx& wtx = (*it).second;
    ...
    BOOST_FOREACH(const PAIRTYPE(CPepeCoinAddress, string)& item, pwalletMain->mapAddressBook)
    {
        ...
        map<CPepeCoinAddress, tallyitem>::iterator it = mapTally.find(address);
...
Value listtransactions(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 4)
        throw runtime_error(
            ...
            "    \"amount\": x.xxx,          (numeric) The amount in PEPE. This is negative for the 'send' category, and for the\n"
...
Value listsinceblock(const Array& params, bool fHelp)
{
    if (fHelp)
       throw runtime_error(
            ...
            "    \"amount\": x.xxx,          (numeric) The amount in PEPE. This is negative for the 'send' category, and for the 'move' category for moves \n"
...
Value walletpassphrase(const Array& params, bool fHelp)
{
    if (pwalletMain->IsCrypted() && (fHelp || params.size() < 2 || params.size() > 3))
        throw runtime_error(
            ...
            "This is needed prior to performing transactions related to private keys such as sending PEPE\n"
            "\nArguments:\n"
...
Value encryptwallet(const Array& params, bool fHelp)
{
    if (!pwalletMain->IsCrypted() && (fHelp || params.size() != 1))
        throw runtime_error(
            ...
            + HelpExampleCli("signmessage", "\"pepecoinaddress\" \"test message\"") +
    ...
    StartShutdown();
    return "wallet encrypted; PepeCoin server stopping, restart to run with encrypted wallet. The keypool has been flushed, you need to make a new backup.";
}
...
Value settxfee(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 1)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. amount         (numeric, required) The transaction fee in PEPE/kB rounded to the nearest 0.00000001\n"
            "\nResult\n"
...
Value getnewstealthaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "Returns a new PepeCoin stealth address for receiving payments anonymously.  ");
...
Value sendtostealthaddress(const Array& params, bool fHelp)
{
    ...
    {
        result.push_back(Pair("result", "Invalid PepeCoin stealth address."));
        return result;
    };
