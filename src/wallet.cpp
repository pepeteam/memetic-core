...
bool CWallet::LoadCScript(const CScript& redeemScript)
{
    ...
    {
        std::string strAddr = CPepeCoinAddress(redeemScript.GetID()).ToString();
...
// hopefully we dont need this anymore but just in case
...
//     tempupdate - this fix was added to 2.x daemon
//        else
    ...
    while (fRepeat)
    {
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CWalletTx)& item, mapWallet)
        {
            ...
            if (txdb.ReadTxIndex(wtx.GetHash(), txindex))
            {
                ...
                if (fUpdated)
                {
                    LogPrintf("ReacceptWalletTransactions found spent coin %s PEPE %s\n", FormatMoney(wtx.GetCredit(ISMINE_ALL)), wtx.GetHash().ToString());
                    wtx.MarkDirty();
                    wtx.WriteToDisk();
                }
            }
            ...
            while (true)
            {
                ...
                if (!SelectCoins(nTotalValue, wtxNew.nTime, setCoins, nValueIn, coinControl, coin_type, useIX))
                {
                    if(coin_type == ALL_COINS) {
                        ...
                        strFailReason = _(" Unable to locate enough Darksend non-denominated funds for this transaction that are not equal 1000 PEPE.");
                    } else {
                ...
                if (nChange > 0)
                {
                    ...
                    // change transaction isn't always pay-to-pepecoin-address
                    CScript scriptChange;
...
bool CWallet::UnlockStealthAddresses(const CKeyingMaterial& vMasterKeyIn)
{
    ...
    for (; mi != mapCryptedKeys.end(); ++mi)
    {
        ...
        if (fDebug)
        {
            ...
            CPepeCoinAddress coinAddress(keyID);
...
bool CWallet::SendStealthMoneyToDestination(CStealthAddress& sxAddress, int64_t nValue, std::string& sNarr, CWalletTx& wtxNew, std::string& sError, bool fAskFee)
{
    ...
    CPepeCoinAddress addrTo(ckidTo);
...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, int64_t nFees, CTransaction& txNew, CKey& key)
{
    ...
    int64_t nReward;
    {
        ...
        int64_t nCoinValue;
        CTxDB txdb("r");
        if (!txNew.GetCoinAge(txdb, pindexPrev, nCoinAge, nCoinValue))
            return error("CreateCoinStake : failed to calculate coin age");

        nReward = GetProofOfStakeReward(pindexPrev, nCoinAge, nFees, nCoinValue);
        if (nReward <= 0)
            return false;
    ...
    if ( Params().NetworkID() == CChainParams::TESTNET ){
        if (pindexPrev->nHeight+1 > PEPE_KEKDAQ_MID_HEIGHT_TESTNET ){
            bMasterNodePayment = true;
        }
    }else{
        if (pindexPrev->nHeight+1 > PEPE_KEKDAQ_MID_HEIGHT){
            bMasterNodePayment = true;
        }
    }
    ...
    if (pindexPrev->nHeight+1 == PEPE_IPFSMN_FNL_HEIGHT)
        devPayment = PEPE_DEV_GRANT_FINAL;
    if (pindexPrev->nHeight == PEPE_KEKDAQ2_HEIGHT)
        devPayment = PEPE_DEV_GRANT;            
    
    // int64_t blockValue = nCredit;   from port rebase
    int64_t masternodePayment = (nReward - (3 * devPayment)) * 0.375;   // 37.5% to MN
    
    //GetMasternodePayment(pindexPrev->nHeight+1, nReward);
    ...
    if(!hasPayment && txNew.vout.size() == 6) // 2 stake outputs, stake was split, plus 3 dev payments
        ...
        else if(hasPayment && txNew.vout.size() == 7) // 2 stake outputs, stake was split, mn payment plus 3 dev payments
        {
            ...
            txNew.vout[payments-3].nValue = devPayment;
            blockValue -= (3 * devPayment);
            txNew.vout[payments-4].nValue = masternodePayment;
...
bool CWallet::SetAddressBookName(const CTxDestination& address, const string& strName)
{
    ...
    if (!fFileBacked)
        return false;
    return CWalletDB(strWalletFile).WriteName(CPepeCoinAddress(address).ToString(), strName);
}
...
bool CWallet::DelAddressBookName(const CTxDestination& address)
{
    ...
    if (!fFileBacked)
        return false;
    CWalletDB(strWalletFile).EraseName(CPepeCoinAddress(address).ToString());
    return CWalletDB(strWalletFile).EraseName(CPepeCoinAddress(address).ToString());
}
